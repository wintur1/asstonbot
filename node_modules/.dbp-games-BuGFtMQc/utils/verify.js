...
async function verify(options) {
    return new Promise(async (res, rej) => {
        ...
        const btn1 = new MessageButton().setLabel(options.buttons?.accept || 'Akceptuj').setStyle('SUCCESS').setCustomId('accept')
        const btn2 = new MessageButton().setLabel(options.buttons?.reject || 'Odrzuc').setStyle('DANGER').setCustomId('reject')
