...
module.exports = class TicTacToe {
    constructor(options = {}) {
        ...
        if (!options.embed.title) options.embed.title = 'Kółko i Krzyżyk';
        if (typeof options.embed.title !== 'string')  throw new TypeError('INVALID_TITLE: Embed Title must be a string.')
        if (!options.embed.overTitle) options.embed.overTitle = 'Koniec Gry';
        ...
        if (!options.askMessage) options.askMessage = 'Hey {opponent}, {challenger} Wyzywa cię na pojedynek kółko i krzyżyk';
        if (typeof options.askMessage !== 'string')  throw new TypeError('ASK_MESSAGE: Ask Messgae must be a string.')
        if (!options.cancelMessage) options.cancelMessage = 'Użytkownik odrzucił pojedynek!';
